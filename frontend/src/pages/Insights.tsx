...
const Insights: React.FC = () => {
  const { data, isLoading, error, refetch, isFetching } = useQuery({
    ...
    staleTime: 30 * 60 * 1000, // cache for 30 min to avoid hammering Gemini API
    retry: 1,
  });
    ...
    <div className="space-y-6">
      ...
      {isLoading && (
        <div className="space-y-4 animate-pulse">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-40 bg-carbon-100 rounded-xl" />
          ))}
          <div className="text-center text-carbon-500 text-sm flex items-center justify-center gap-2">
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-red-800">Could not load insights</h3>
