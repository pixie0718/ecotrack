...
import { CATEGORY_ICONS, type CarbonCategory } from '@/types/carbon.types';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-carbon-500 mt-1">Track your daily activities and calculate their carbon impact</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-carbon-500">Your last 20 logged activities</p>
          </CardHeader>

          {isLoading ? (
            <div className="space-y-3 animate-pulse">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="h-14 bg-carbon-50 rounded-lg" />
              ))}
            </div>
          ) : data?.data.length === 0 ? (
            <div className="text-center py-10 text-carbon-400">
            ...
            <div className="space-y-2">
              {data?.data.map((activity) => (
                <div
                  key={activity.id}
                  className="flex items-center justify-between p-3 bg-carbon-50 rounded-lg hover:bg-carbon-100 transition-colors group"
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1.5 text-carbon-400 hover:text-red-500 hover:bg-red-50 rounded transition-all"
